...
SOUND_FOLDER="./sound"
# --- NOWOŚĆ: Ścieżka do pliku dźwiękowego ---
ERROR_SOUND="$SOUND_FOLDER/error.wav"

# --- NOWOŚĆ: Funkcja do odtwarzania dźwięku błędu ---
# Sprawdza, czy plik istnieje i czy dostępne jest polecenie `paplay` (dla PulseAudio)
# lub `aplay` (dla ALSA) jako alternatywa.
play_error_sound() {
...
if [[ ! "$MAILS_TO_MARK" =~ ^[1-9][0-9]*$ ]]; then
  # --- NOWOŚĆ: Odtwórz dźwięk przy błędzie walidacji ---
  play_error_sound
...
if [[ -s "$SUMMARY_ERR" ]]; then
  # --- NOWOŚĆ: Odtwórz dźwięk, jeśli w podsumowaniu są błędy ---
  play_error_sound
